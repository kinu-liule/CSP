...
const MIGRATIONS = [
  ...
  "ALTER TABLE soar_incidents ADD COLUMN IF NOT EXISTS department_id VARCHAR(64)",
  // Data Security
...
const INDEXES = [
  ...
  'CREATE INDEX IF NOT EXISTS idx_dept_grc_policies ON policies(department_id)',
...
async function run() {
  ...
  try {
    console.log('Applying department_id columns...\n');
    ...
    for (const sql of MIGRATIONS) {
      try {
        ...
        if (err.message.includes('does not exist')) {
          console.log(`  SKIP: Table not found: ${sql.substring(0, 80)}...`);
        } else {
    ...
    console.log(`\nDone: ${success} columns added, ${fail} failures`);
  } catch (err) {
